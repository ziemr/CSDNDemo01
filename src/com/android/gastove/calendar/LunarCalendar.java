package com.android.gastove.calendar;
...
public class LunarCalendar {
	 private int year;  //nongli的年fen
	 private int month;
	 private int day;
	 private String lunarMonth;  //nongli的月fen
	 private boolean leap;
	 public int leapMonth = 0;  //run的是na个月
...
//	          "1226 毛zedongdan辰ji念"
	 ...
	 // ====== chuan回nongli y年闰哪个月 1-12 , 没闰chuan回 0
	 final private static int leapMonth(int y) {
	  return (int) (lunarInfo[y - 1900] & 0xf);
	 }

	 // ====== chuan回nongli y年m月的总天数
	 ...
	 // ====== chuan入 月日的offset chuan回干支, 0=甲子
	 ...
	 /** */
	 /**
	  * chuan出y年m月d日对应的nongli. yearCyl3:nongli年与1864的相差数 ? monCyl4:从1900年1月31日以来,闰月数
	  * dayCyl5:与1900年1月31日相差的天数,再加40 ?
	  *
	  * isday: 这个参数为false---日期为节假日时，阴li日期就返回节假日 ，true---不管日期是否为节假日依然返回这天对应的阴li日期
	  * @param cal
	  * @return
	  */
	 public String getLunarDate(int year_log, int month_log, int day_log, boolean isday) {
	  ...
	  // nongli年份
	  year = iYear;
	        setYear(year);  //设置公li对应的nongli年份

	  yearCyl = iYear - 1864;
	  leapMonth = leapMonth(iYear); // 闰哪个月,1-12
	  leap = false;

	  // 用当年的天数offset,逐个减去每月（nongli）的天数，求出当天是本月的第几天
	  ...
	  month = iMonth;
	  setLunarMonth(chineseNumber[month - 1] + "月");  //设置对应的阴li月份
	  day = offset + 1;

	  if(!isday){
	   //如果日期为节假日则阴li日期则返回节假日
