...
//		intent.setClass(RemarkActivity.this, WarHosDataLeaf.class);
...
//			DragGridViewType.setItems(items);
	...
	public void reflashItems() {
		Log.i(TAG, "reflashItems start");
 		items.clear();
 		adapterCount = mDbOperator.gettableCount(Const.TABLE_remark);
    	for (int i = 0; i < adapterCount; i++) {
        	String name = mDbOperator.getRemarkName(i);
            items.add(name);
        }
    	if (!EDIT_FLAG) 
    	   items.add("+");
		...
		alertDialog.setPositiveButton(android.R.string.ok,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if(mPosition >= adapterCount) {
							mDbOperator.insertRemark(et_dialog_confirmphoneguardpswd.getText().toString().trim());
						} else {
							mDbOperator.updateRemarkName(Utils.toRekTypeID(mPosition), et_dialog_confirmphoneguardpswd.getText().toString().trim());
						}
