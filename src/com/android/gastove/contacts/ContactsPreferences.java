...
public final class ContactsPreferences extends ContentObserver {
    ...
    String SORT_ORDER = "android.contacts.SORT_ORDER";
    String DISPLAY_ORDER = "android.contacts.DISPLAY_ORDER";
    ...
    private int getDefaultSortOrder() {
        if (mContext.getResources().getBoolean(R.bool.config_default_sort_order_primary)) {
            return 1;// ContactsContract.Preferences.SORT_ORDER_PRIMARY;
        } else {
            return 2; //ContactsContract.Preferences.SORT_ORDER_ALTERNATIVE;
        }
    }
        ...
        if (mSortOrder == -1) {
            try {
                mSortOrder = Settings.System.getInt(mContext.getContentResolver(),
                		"android.contacts.SORT_ORDER");// SORT_ORDER);
    ...
    private int getDefaultDisplayOrder() {
        if (mContext.getResources().getBoolean(R.bool.config_default_display_order_primary)) {
            return 1;//ContactsContract.Preferences.DISPLAY_ORDER_PRIMARY;
        } else {
            return 2;//ContactsContract.Preferences.DISPLAY_ORDER_ALTERNATIVE;
        }
    }
    ...
    public void registerChangeListener(ChangeListener listener) {
        ...
        contentResolver.registerContentObserver(
                Settings.System.getUriFor(
                        SORT_ORDER), false, this);
        contentResolver.registerContentObserver(
                Settings.System.getUriFor(
                        DISPLAY_ORDER), false, this);
    }
